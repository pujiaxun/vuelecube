<template lang="pug">
  .layout-menu
    Menu(:theme='configs.themeStyle'
      :active-name='$router.currentRoute.name'
      :style='menuStyle'
      @on-select='onSelectHandler')

      .layout-logo-left(@click='toggleClick') Vuelecube
      MenuItem(v-for="menu in menus" :key='menu.name' :name='menu.name')
        span.menu-icon-container(:style='iconStyle')
          Icon.menu-icon(:type='menu.icon' :size='iconSize')
        transition(name='fade')
          span.menu-text(v-if='isExpending') {{ menu.title }}
</template>
